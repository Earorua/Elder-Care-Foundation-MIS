...
FILTER_DEFS = {
    ...
    'batch_type':       {'label': 'Batch Type',       'domain': 'gift', 'type': 'enum',   'sql': 'gb.batch_type',
                         'options_query': "SELECT DISTINCT batch_type FROM gift_batch WHERE batch_type != '' ORDER BY batch_type"},
    'distribution_date':{'label': 'Distribution Date','domain': 'gift', 'type': 'daterange', 'sql': 'gd.distribution_date'},
    # Finance filters
    ...
    'schedule_type_filter': {'label': 'Schedule Type', 'domain': 'schedule', 'type': 'enum', 'sql': 's.schedule_type',
                         'options_query': "SELECT DISTINCT schedule_type FROM schedules WHERE schedule_type != '' ORDER BY schedule_type"},
}
...
DIMENSION_DEFS = {
    ...
    'donation_month':   {'label': 'Donation Month',   'domain': 'donor',
                         'sql': "strftime('%Y-%m', d.donation_date)",
                         'alias': 'donation_month'},
    'gift_given_dim':   {'label': 'Gift Given',       'domain': 'donor',
                         'sql': "CASE WHEN d.gift_given IS NOT NULL AND d.gift_given != '' THEN d.gift_given ELSE 'None' END",
                         'alias': 'gift_given'},
    ...
    'payment_type':     {'label': 'Payment Type',     'domain': 'person', 'sql': 'pay.payment_type',
                         'alias': 'payment_type'},
    'payment_year':     {'label': 'Payment Year',     'domain': 'person',
                         'sql': "strftime('%Y', pay.payment_date)",
                         'alias': 'payment_year'},
    'payment_month':    {'label': 'Payment Month',    'domain': 'person',
                         'sql': "strftime('%Y-%m', pay.payment_date)",
                         'alias': 'payment_month'},
    'hire_year':        {'label': 'Hire Year',        'domain': 'person',
                         'sql': "strftime('%Y', p.hire_date)",
                         'alias': 'hire_year'},
    # Event dimensions
    ...
    'event_year':       {'label': 'Event Year',       'domain': 'event',  'sql': "strftime('%Y', e.start_date)",
                         'alias': 'event_year'},
    'event_name_dim':   {'label': 'Event Name',      'domain': 'event',  'sql': 'e.event_name',
                         'alias': 'event_name'},
    'event_month':      {'label': 'Event Month',     'domain': 'event',
                         'sql': "strftime('%Y-%m', e.start_date)",
                         'alias': 'event_month'},
    'event_quarter':    {'label': 'Event Quarter',   'domain': 'event',
                         'sql': "strftime('%Y', e.start_date) || '-Q' || ((CAST(strftime('%m', e.start_date) AS INTEGER) + 2) / 3)",
                         'alias': 'event_quarter'},
    # Gift dimensions
    ...
    'gift_name_dim':    {'label': 'Gift Name',       'domain': 'gift',  'sql': 'g.gift_name',
                         'alias': 'gift_name'},
    'batch_type_dim':   {'label': 'Batch Type',      'domain': 'gift',  'sql': 'gb.batch_type',
                         'alias': 'batch_type'},
    'distribution_reason_dim': {'label': 'Distribution Reason', 'domain': 'gift', 'sql': 'gd.distribution_reason',
                         'alias': 'distribution_reason'},
    'distribution_year':{'label': 'Distribution Year','domain': 'gift',
                         'sql': "strftime('%Y', gd.distribution_date)",
                         'alias': 'distribution_year'},
    'distribution_month':{'label': 'Distribution Month','domain': 'gift',
                         'sql': "strftime('%Y-%m', gd.distribution_date)",
                         'alias': 'distribution_month'},
    # Finance dimensions
    ...
    'finance_year':        {'label': 'Year',          'domain': 'finance',
                            ...
                            'alias': 'finance_year'},
    'finance_name':        {'label': 'Income Name',   'domain': 'finance', 'sql': 'f.name',
                            'alias': 'income_name'},
    'finance_month':       {'label': 'Month',         'domain': 'finance',
                            'sql': "strftime('%Y-%m', f.received)",
                            'alias': 'finance_month'},
    'finance_quarter':     {'label': 'Quarter',       'domain': 'finance',
                            'sql': "strftime('%Y', f.received) || '-Q' || ((CAST(strftime('%m', f.received) AS INTEGER) + 2) / 3)",
                            'alias': 'finance_quarter'},
    # Schedule dimensions
    ...
    'schedule_person_type': {'label': 'Person Type',     'domain': 'schedule', 'sql': 'p.person_type',
                             'alias': 'person_type'},
    'schedule_type_dim':    {'label': 'Schedule Type',   'domain': 'schedule', 'sql': 's.schedule_type',
                             'alias': 'schedule_type'},
    'available_time_dim':   {'label': 'Available Time',  'domain': 'schedule', 'sql': 's.availibile_time',
                             'alias': 'available_time'},
    'shift_year':           {'label': 'Shift Year',      'domain': 'schedule',
                             'sql': "strftime('%Y', s.shift_date)",
                             'alias': 'shift_year'},
    'schedule_person_role': {'label': 'Person Role',     'domain': 'schedule', 'sql': 'p.role_name',
                             'alias': 'person_role'},
}
...
METRIC_DEFS = {
    ...
    'batch_count':           {'label': 'Batch Count',     'domain': 'gift',
                              'sql': 'COUNT(DISTINCT gb.batch_id)', 'format': 'integer'},
    'total_batch_qty':       {'label': 'Total Batch Qty', 'domain': 'gift',
                              'sql': 'SUM(gb.batch_quantity)', 'format': 'integer'},
    # Finance metrics
    ...
    'attendance_rate':  {'label': 'Attendance Rate',   'domain': 'schedule',
                         ...
                         'format': 'percent_val'},
    'avg_overtime':     {'label': 'Avg Overtime',      'domain': 'schedule',
                         'sql': 'ROUND(AVG(s.overtime), 1)', 'format': 'currency'},
}
